/**
 * prisma/seed.js — development seed
...
async function main() {
  ...
  const criticalEligibleIds   = heroes.filter((h) => h.power === 'flight' || h.power === 'strength').map((h) => h.id);
  const busyHeroIds            = new Set();

  // dane + przypisania kategorii generowane w jednym przebiegu
  const incidentData         = [];
  const incidentCategoryMeta = []; // równoległa tablica: kategorie per incydent
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (status !== 'open') {
      // krytyczny → wymagany lot lub siła
    ...
    incidentData.push({ location: faker.location.streetAddress(), district: faker.location.county(), level, status, heroId, assignedAt, resolvedAt, resolutionMinutes });

    // 1–3 unikalne kategorie per incydent
  ...
  // masowy insert tabeli pośredniej
  const icRecords = [];
