...
async function assignHero(incidentId, heroId) {
  ...
  // transakcja — auto-commit/rollback
  return prisma.$transaction(async (tx) => {
    // tx.* wymagane — nie prisma.*
...
async function resolveIncident(incidentId) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const resolved = await incidentRepository.resolveWithTx(incidentId, resolutionMinutes, tx);
