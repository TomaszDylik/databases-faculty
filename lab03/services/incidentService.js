...
async function assignHero(incidentId, heroId) {
  ...
  return sequelize.transaction(async (t) => {
    const incident = await incidentRepository.findByIdForUpdate(incidentId, t);
    if (!incident) {
      throw new NotFoundError(`Incident with id ${incidentId} not found`);
    }
    ...
    const hero = await heroRepository.findByIdForUpdate(heroId, t);
    if (!hero) {
    ...
    const updated = await incidentRepository.assignHero(incidentId, heroId, t);
    await heroRepository.updateStatus(heroId, 'busy', t);
    return updated;
  });
}
...
async function resolveIncident(incidentId) {
  ...
  return sequelize.transaction(async (t) => {
    ...
    // resolve() calls instance.update() which triggers afterUpdate hook — missions_count incremented atomically
    const resolved = await incidentRepository.resolve(incidentId, t);

    if (incident.hero_id) {
      await heroRepository.updateStatus(incident.hero_id, 'available', t);
    }
